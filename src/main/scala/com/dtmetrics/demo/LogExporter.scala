...
object LogExporter {
  ...
  def emit(tick: Tick, config: DemoConfig, sock: DatagramSocket): ZIO[Any, Throwable, Unit] =
    for {
      ...
      _ <- ZIO.logInfo(s"[OTEL-LOG] tick: cn_otel_log_cpu=${tick.cpu} cn_otel_log_mem=${tick.mem} cn_otel_log_lat=${tick.lat} svc=${config.serviceName}")
      _ <- ZIO.attempt {
        val ts     = Instant.now().toString
        val msg    = s"[INFO] tick: cn_syslog_cpu=${tick.cpu} cn_syslog_mem=${tick.mem} cn_syslog_lat=${tick.lat} svc=${config.serviceName}"
